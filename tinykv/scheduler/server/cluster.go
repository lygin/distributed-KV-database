...
func (c *RaftCluster) processRegionHeartbeat(region *core.RegionInfo) error {
	// Your Code Here (3C).
	ep := region.GetRegionEpoch()
	if ep == nil {
		return errors.Errorf("No epoch")
	}
	origin := c.GetRegion(region.GetID())
	if origin == nil {
		regions := c.ScanRegions(region.GetStartKey(), region.GetEndKey(), -1)
		for _, old := range regions {
			if isStaleRegion(old, region) {
				return ErrRegionIsStale(region.GetMeta(), old.GetMeta())
			}
		}
	} else if isStaleRegion(origin, region) {
...
func isStaleRegion(origin *core.RegionInfo, region *core.RegionInfo) bool {
	oep := origin.GetRegionEpoch()
	rep := region.GetRegionEpoch()

	if rep.GetVersion() < oep.GetVersion() || rep.GetConfVer() < oep.GetConfVer() {
		return true
	}
	return false
}
